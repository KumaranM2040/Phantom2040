  ...
  return {
    ...
    controller: function($scope, $http){
      ...
      $http.get(path).success(function(data) {
        ...
        // BEGIN: Remove the script added by the live server
        let posStart = data.indexOf("  <!-- Code injected by live-server -->");
        let posEnd = data.indexOf("</script>", posStart) + "</script>".length;
        if (posStart > 1){          
          data = data.slice(0, posStart -1 ) + data.slice(posEnd)
        }        
        // END
        $scope.content = data;
      });
