...
class CloudFlareAPI {
   constructor(email, key, zoneId, accountId, domainName, publicIpUrl) {
   ...
   async updateCloudflareDNSA(currentPublicIP) {
      ...
      try {
         ...
         console.log(
            'CloudFlare DNSA record Ip Address for ' +
               this.domainName +
               ' is ' +
               dnsARecord
         );
         ...
         if (dnsARecord !== currentPublicIP && currentPublicIP.length > 0) {
            ...
            const url =
               this.CloudFlareApiUrl + this.zoneId + '/dns_records/' + dnsId;
