...
async function checkForUpdates () {
   ...
   if (currentPublicIp === hostPublicIpAddress) {
      console.log(new Date().toLocaleString() + ' Our Public IP has not changed since the last time we checked. No need to update Cloudflare');
      return;
   } else {
   ...
   if (currentPublicIp !== dnsAIpAddress) {
      ...
      console.log(new Date().toLocaleString() + ' Cloudflare DNSA Ip Address and our current public IP Address are in sync. Nothing to do !!');
   }
}
checkForUpdates();
