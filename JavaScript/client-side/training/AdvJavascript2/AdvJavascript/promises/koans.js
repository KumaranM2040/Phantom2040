...
runner.addKoan(`What value is passed to the promise continuation?`, async ({ expect }) => {
    // let promise = new Promise((resolve, reject) => {
    //     resolve('Hello');
    // }).then((value) => {
    //     expect(value, 'Hello');
...
runner.addKoan(`What value is passed to the promise catch?`, async ({ expect }) => {
    // let promise = new Promise((resolve, reject) => {
    //     reject('Help');
    // }).catch((error) => {
    //     expect(error, 'Help');
    ...
    let error = await 'Help'

    await expect(error, 'Help');
});
...
runner.addKoan(`Is a resolved Promise continued before/after a queued microtask?`, async ({ expect }) => {
...
runner.addKoan(`Is a resolved Promise continued before/after the next microtask?`, async({ expect }) => {
    let microtaskExecuted = false;
...
runner.addKoan(`Is a resolved Promise continued before/after a queued task?`, async({ expect }) => {
...
runner.addKoan(`Is a resolved Promise continued before/after the next task?`, async ({ expect }) => {
    let taskExecuted = false;
...
runner.addKoan(`Is a resolved Promise continued before/after a queued animation frame?`, async({ expect, done }) => {
    let animationExecuted = false;

    async function callRALF() {
